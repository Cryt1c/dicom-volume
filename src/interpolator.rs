...
mod tests {
    ...
    fn test_bilinear_interpolate_midpoint() {
        let data = Array2::from_shape_vec((2, 2), vec![0u16, 2, 4, 6]).unwrap();
        let view = data.view();
        let result = Interpolator::bilinear_interpolate(&view, 0.5, 0.5);
        // Average of all four corners: (0 + 2 + 4 + 6) / 4 = 3
        assert_eq!(result, 3);
    }

    #[test]
    fn test_bilinear_interpolate_horizontal_midpoint() {
        let data = Array2::from_shape_vec((2, 2), vec![0u16, 4, 0, 4]).unwrap();
        let view = data.view();
        let result = Interpolator::bilinear_interpolate(&view, 0.0, 0.5);
        // Midpoint between 0 and 4
        assert_eq!(result, 2);
    }

    #[test]
    fn test_bilinear_interpolate_vertical_midpoint() {
        let data = Array2::from_shape_vec((2, 2), vec![0u16, 0, 4, 4]).unwrap();
        let view = data.view();
    ...
    fn test_bilinear_interpolate_quarter_point() {
        let data = Array2::from_shape_vec((2, 2), vec![0u16, 4, 8, 12]).unwrap();
        let view = data.view();
    ...
    fn test_bilinear_interpolate_boundary_clamping() {
        ...
        // Should clamp to (2, 2) which is 9
        assert_eq!(result, 9);
    }

    #[test]
    fn test_bilinear_interpolate_single_pixel() {
        let data = Array2::from_shape_vec((1, 1), vec![42u16]).unwrap();
        let view = data.view();
        let result = Interpolator::bilinear_interpolate(&view, 0.0, 0.0);
        assert_eq!(result, 42);
    }
